...
class ProductDAO(conn: Connection) {
  ...
  def edit(product: Product): Product = {
    val q =
      """
      |UPDATE product
      |SET  name = ?, url = ?, quantity = ?, with_box = ?,
      |     delivery_date = ?, detail = ?, image = ?, owner = ?
      |WHERE courier_id = ? AND id = ?;
      |""".stripMargin
    ...
    stmt.setInt(4, product.withBox)
    stmt.setDate(5, DateUtil.parseDate(product.deliveryDate))
    stmt.setString(6, product.detail.getOrElse(""))
    stmt.setString(7, product.image)
    stmt.setString(8, product.owner.getOrElse(""))
    stmt.setInt(9, product.courierId)
    stmt.setInt(10, product.id.get)
